...
1. resource/static 的測試環境為 1920 x 1080，模擬器採全螢幕，不同環境可能會導致抓取圖片失敗，建議使用 ui.exe 的功能重新抓取圖
2. 支援各種視窗大小，但更換後建議再次抓圖降低誤判可能
...
2. 點擊 game.exe
3. 點擊 ui.exe
4. 選擇模擬器，選擇前台/後台，輸入模擬器名稱
5. 第一次執行請點選自動抓圖 (當進入遊戲時，若有跳出任何訊息 (e.g. 儲值)，請立即點選關閉，以免影響抓圖流程)
6. 設定好 ui.exe 後點選開始
